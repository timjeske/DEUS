...
To create the count table for differential expression analysis, [createCountTableFromFastQs](../reference/createCountTableFromFastQs.html) counts all unique sequences in each FASTQ file given in your `pheno_info`. To remove low expressed sequences and reduce the size of the count table, we keep only sequences with an average sequence count larger than 1.
